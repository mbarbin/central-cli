...
(*_ Notice: The unified-diff renderer and the [Equal] / [Line] / [compute] API
  ...
  lives in [merge3.mli], vendored from gazagnaire/ocaml-merge3); the rest of
  the interface derives from windtrap.

  List of changes relative to windtrap:

  - Applied local project ocamlformat (janestreet profile), including
    reordering [Line.t]'s constructors and dropping their per-constructor
    doc comments.
  - Added an optional [?color:bool] flag to [diff] for ANSI coloring.
